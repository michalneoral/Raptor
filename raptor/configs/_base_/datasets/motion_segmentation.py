...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='Normalize', **img_norm_cfg),
            dict(type='Pad', size_divisor=32),
            dict(type='ImageToTensor_Custom', keys=['img', 'img2', 'calib_K', 'calib_K_2', 'calib_baseline']),
            # dict(type='DefaultFormatBundle_Custom'),
            dict(type='Collect_Custom', keys=['img', 'img2', 'calib_K', 'calib_K_2', 'calib_baseline']),
        ])
]
test_pipeline_kitti = [
    dict(type='LoadTwoImagesFromFiles'),
...
data = dict(
    ...
    train=dict(
        ...
        datasets=[
            dict(
                type=dataset_type,
                ann_file=data_root +
                'motion_segmentation_coco_format/moving_objects_driving_final_train.json',
            ...
            dict(
                type=dataset_type,
                ann_file=data_root +
                'motion_segmentation_coco_format/moving_objects_driving_clean_train.json',
                img_prefix=data_root,
                seg_prefix=data_root + 'binary_motion_segmentation_png/',
                pipeline=train_pipeline),
            dict(
                type=dataset_type,
                ann_file=data_root +
                'motion_segmentation_coco_format/moving_objects_ft3d_clean_train.json',
                img_prefix=data_root,
                seg_prefix=data_root + 'binary_motion_segmentation_png/',
                pipeline=train_pipeline),
            dict(
                type=dataset_type,
                ann_file=data_root +
                'motion_segmentation_coco_format/moving_objects_moonka_clean_train.json',
                img_prefix=data_root,
                seg_prefix=data_root + 'binary_motion_segmentation_png/',
                pipeline=train_pipeline),
        ]
        ...
        datasets=[
            dict(
                ...
                ann_file=data_root_kitti +
                         'motion_segmentation_coco_format/moving_objects_kitti_original_all.json',
                img_prefix=data_root_kitti,
                pipeline=test_pipeline_kitti),
            dict(
                type=dataset_type_eval_davis,
                ann_file=data_root_davis +
                         'motion_segmentation_coco_format/moving_objects_davis_small_val.json',
                img_prefix=data_root_davis + 'DAVIS17/',
                pipeline=test_pipeline_davis),
            dict(
                type=dataset_type_eval_driving,
                ann_file=data_root +
                         'motion_segmentation_coco_format/moving_objects_driving_final_small_val.json',
                img_prefix=data_root,
                pipeline=test_pipeline),
            dict(
                type=dataset_type_eval_ft3d,
                ann_file=data_root +
                         'motion_segmentation_coco_format/moving_objects_ft3d_clean_small_val.json',
                img_prefix=data_root,
                pipeline=test_pipeline),
            dict(
                type=dataset_type_eval_moonka,
                ann_file=data_root +
                         'motion_segmentation_coco_format/moving_objects_moonka_clean_small_val.json',
                img_prefix=data_root,
                pipeline=test_pipeline),
        ]
    ),
    test=dict(
        ...
        datasets=[
            ...
            #     type=dataset_type_eval_davis,
            #     ann_file=data_root_davis +
            #              'motion_segmentation_coco_format/moving_objects_davis_small_all.json',
            #     img_prefix=data_root_davis + 'DAVIS17/',
            #     pipeline=test_pipeline_davis),
            # dict(
            #     type=dataset_type_eval_driving,
